...
public class EventTree extends DefaultMutableTreeNode {
    ...
    public EventTree(Scanner scanner) {
        ...
        this.events = new Event[] {
            ...
            new Battle(new GeorgeStPierre("George St-Pierre", 36, 12, 10), scanner)
        };
    }
    ...
    public EventNode createTree() {
        ...
        nodes[0].add(nodes[2]); //Kenneth -> shop
        nodes[1].add(nodes[3]); //bater o dedo -> Max Holloway
        nodes[1].add(nodes[4]); //Bater o dedo -> Training camp
        nodes[2].add(nodes[5]); //Shop -> FrancisNgannou
        nodes[3].add(nodes[6]); //MaxHolloway -> Shop
        nodes[3].add(nodes[7]); //Max Holloway -> Training Camp
        nodes[4].add(nodes[8]); //Training camp -> John Jones
        nodes[5].add(nodes[9]); //Francis Ngannou -> Training Camp
        nodes[5].add(nodes[10]); //Francis Nganoou -> Balada
        nodes[6].add(nodes[11]); //Shop -> Israel Adesanya
        nodes[7].add(nodes[12]); //Training Camp-> Khabib
        nodes[9].add(nodes[13]); //Training camp -> Connor McGregor
        nodes[10].add(nodes[14]); //Balada -> George St-Pierre
